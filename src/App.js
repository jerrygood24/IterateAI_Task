  ...
  const fetchProduct = async (productIdea) => {
    ...
    const options = {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Generate existing product related to:  ${productIdea} , just show the product name and features without any heading.` },
        ],
    ...
    try {
      ...
      if (responseData.choices && responseData.choices.length > 0) {
        const recommendations = responseData.choices.map(choice => choice.message.content.trim());
        setProductList(recommendations); // Set the entire array of recommendations
